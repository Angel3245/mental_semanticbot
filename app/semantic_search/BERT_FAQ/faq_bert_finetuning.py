...
class FAQ_BERT_Finetuning(object):
    ...
    def __init__(self, loss_type="triplet", query_type='faq', neg_type='simple', version="1.1", epochs=4, batch_size=32, 
                 pre_trained_name='distilbert-base-uncased', evaluation_steps=1000, test_size=0.20, num_labels=1, learning_rate=5e-5,
                 adam_epsilon=1e-8, max_seq_length=128):
        ...
        if self.loss_type == "triplet":
          ...
          word_embedding_model = models.Transformer(self.pre_trained_name, max_seq_length=self.max_seq_length)
          ...
          self.bert_model = SentenceTransformer(modules=[word_embedding_model, pooling_model])
            ...
            self.bert_model = CrossEncoder(self.pre_trained_name, num_labels=self.num_labels, max_length=self.max_seq_length)
    ...
    def split_train_val_sets(self, df):
        ...
        """
        train, val = train_test_split(df, test_size=self.test_size)
        return train, val
    ...
    def create_model(self, df, output_path):
        ...
        try:
            ...
            self.train_df, self.val_df = self.split_train_val_sets(df)
            
            # generate triplets for train, test, validation
            train_samples = self.generate_triplets(self.train_df)
            val_samples = self.generate_triplets(self.val_df)
    ...
    def create_model_eval(self, df, output_path):
        """ Finetune BERT model on FAQ dataset, generate model and evaluate it at given path
